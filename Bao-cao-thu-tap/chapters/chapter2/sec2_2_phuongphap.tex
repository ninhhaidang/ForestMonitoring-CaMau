...
Giai đoạn đầu tiên trong quy trình là thu thập và tiền xử lý dữ liệu ảnh vệ tinh. Quá trình này được thực hiện trên nền tảng Google Earth Engine, bao gồm các bước: (1) truy xuất dữ liệu Sentinel-2 và Sentinel-1 cho khu vực nghiên cứu tại hai thời điểm (tháng 01/2024 và 02/2025), (2) lọc theo không gian và thời gian, (3) loại bỏ ảnh hưởng của mây sử dụng bộ sưu tập \textit{S2\_CLOUD\_PROBABILITY}, (4) trích xuất các kênh phổ cần thiết và tính toán các chỉ số thực vật, và (5) xuất kết quả ra định dạng GeoTIFF với độ phân giải 10m. Chi tiết về tiêu chí lựa chọn ảnh, quy trình xử lý trên GEE, và cơ sở khoa học trong việc lựa chọn các kênh phổ và chỉ số thực vật đã được trình bày chi tiết trong Mục~\ref{sec:data} (Phần Dữ liệu).
...
Kết quả là mỗi mẫu có kích thước (3, 3, 27) — tương ứng với kích thước điểm ảnh và số kênh đặc trưng.
...
Chiến lược chia dữ liệu được thiết kế theo khuyến nghị của Roberts et al. \citeen{roberts2017} về kiểm định chéo cho dữ liệu không gian. Ban đầu tách 20\% dữ liệu làm tập kiểm tra cố định (526 mẫu) — tập này không được sử dụng trong quá trình huấn luyện hay tinh chỉnh siêu tham số. Sau đó áp dụng kiểm định chéo 5 phần trên 80\% còn lại (2,104 mẫu) để đánh giá độ ổn định của mô hình và tìm kiếm siêu tham số tối ưu. Tiếp theo sau khi xác định siêu tham số tối ưu, huấn luyện mô hình cuối cùng trên toàn bộ 80\% dữ liệu để tận dụng tối đa dữ liệu huấn luyện. Cuối cùng đánh giá mô hình cuối cùng trên 20\% tập kiểm tra để báo cáo kết quả. Cách phân chia dữ liệu này đảm bảo tỷ lệ các lớp (Rừng ổn định, Mất rừng, Phi rừng, Phục hồi rừng) được duy trì đồng đều trong cả tập huấn luyện và kiểm tra.
...
Sau khi hoàn tất huấn luyện, mô hình tốt nhất (có validation loss thấp nhất) được lưu lại dưới dạng checkpoint sử dụng thư viện PyTorch. Checkpoint chứa các thành phần quan trọng bao gồm: (1) \textit{model\_state\_dict} --- trọng số của tất cả các lớp trong mạng nơ-ron, (2) \textit{optimizer\_state\_dict} --- trạng thái của thuật toán tối ưu AdamW, (3) các chỉ số hiệu suất tốt nhất (\textit{best\_val\_loss}, \textit{best\_val\_acc}), và (4) lịch sử huấn luyện (\textit{history}) ghi lại giá trị loss và accuracy qua từng epoch. Mô hình được lưu dưới định dạng \texttt{.pth} tại đường dẫn \texttt{results/models/cnn\_final\_model.pth}. Bên cạnh đó, các tham số chuẩn hóa (mean và std) được lưu riêng để đảm bảo dữ liệu mới trong giai đoạn dự đoán được chuẩn hóa theo cùng phân phối với tập huấn luyện.

\subsection{Áp dụng mô hình}

Sau khi huấn luyện, mô hình được áp dụng để phân loại toàn bộ vùng nghiên cứu với khoảng 16.2 triệu điểm ảnh hợp lệ, tương đương diện tích 162,468 ha. Trước khi thực hiện dự đoán, mô hình đã lưu cần được tải lại vào bộ nhớ. Quá trình này bao gồm: (1) khởi tạo kiến trúc mô hình CNN với cùng cấu hình đã sử dụng trong huấn luyện, (2) tải checkpoint từ file \texttt{.pth} sử dụng hàm \texttt{torch.load()}, (3) nạp trọng số từ \textit{model\_state\_dict} vào mô hình thông qua phương thức \texttt{load\_state\_dict()}, và (4) chuyển mô hình sang chế độ đánh giá (\texttt{eval()}) để vô hiệu hóa các lớp Dropout và BatchNorm. Mô hình sau đó được chuyển sang GPU (nếu có) để tăng tốc độ tính toán.

Quy trình dự đoán được thực hiện như sau.
