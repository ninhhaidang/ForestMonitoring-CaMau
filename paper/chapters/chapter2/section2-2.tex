...
\begin{figure}[H]
\centering
\includegraphics[width=0.9425\textwidth]{chapter2/flowchart.png}
...
\caption{Kiến trúc mô hình mạng nơ-ron tích chậpF được sử dụng trong nghiên cứu}
\label{fig:cnn_architecture}
\end{figure}

Kiến trúc mô hình được xây dựng dựa trên các kỹ thuật đã được chứng minh hiệu quả trong lĩnh vực học sâu, đồng thời tùy chỉnh các siêu tham số để phù hợp với đặc thù bài toán và quy mô dữ liệu. Về các thành phần chuẩn, nghiên cứu sử dụng bộ lọc tích chập kích thước $3 \times 3$ --- kích thước được Simonyan và Zisserman \cite{simonyan2015} chứng minh là hiệu quả nhất trong kiến trúc VGGNet, cho phép tăng chiều sâu mạng với ít tham số hơn so với bộ lọc lớn. Chuẩn hóa theo lô  \cite{ioffe2015} được áp dụng sau mỗi lớp tích chập để ổn định quá trình huấn luyện. Hàm kích hoạt ReLU được sử dụng do tính đơn giản và khả năng giảm thiểu vấn đề triệt tiêu gradient \cite{goodfellow2016}. GAP được đề xuất bởi Lin và cộng sự \cite{lin2014} thay thế cho các lớp kết nối đầy đủ truyền thống, giúp giảm đáng kể số lượng tham số. Kỹ thuật Dropout \cite{srivastava2014} được sử dụng để ngăn ngừa hiện tượng quá khớp.
...
Với bộ dữ liệu gồm 2,630 mẫu và mô hình có 36,676 tham số, tỷ lệ mẫu trên tham số là 72:1 --- tỷ lệ này thấp hơn ngưỡng khuyến nghị thông thường (100:1 đến 1000:1) cho mô hình học sâu \cite{goodfellow2016}, do đó tăng nguy cơ quá khớp. Để ngăn chặn hiện tượng quá khớp --- khi mô hình ghi nhớ dữ liệu huấn luyện thay vì học các quy luật tổng quát --- nghiên cứu áp dụng ba kỹ thuật điều chuẩn: chuẩn hóa theo lô \cite{ioffe2015} sau mỗi lớp mạng, Dropout với tỷ lệ 70\% \cite{srivastava2014}, và phân rã trọng số với hệ số $10^{-3}$. Bảng~\ref{tab:hyperparams} trình bày các siêu tham số huấn luyện.
