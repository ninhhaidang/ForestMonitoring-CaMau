...
Dữ liệu đóng vai trò quyết định trong các mô hình học sâu. Để xây dựng hệ thống giám sát biến động rừng tỉnh Cà Mau, em đã đã thiết lập một quy trình thu thập và xử lý dữ liệu nghiêm ngặt từ các nguồn vệ tinh miễn phí nhưng chất lượng cao của Chương trình Copernicus. Dữ liệu thực địa được Công ty TNHH Tư vấn và Phát triển Đồng Xanh cung cấp trong khuôn khổ đề tài ``Ứng dụng công nghệ viễn thám, ảnh vệ tinh để quản lý, giám sát tài nguyên rừng và sạt lở ven biển trên địa bàn tỉnh Cà Mau'' \citevi{gfd2025}.

Nguồn dữ liệu chính được sử dụng bao gồm ảnh quang học từ vệ tinh Sentinel-2 \citeen{esa2024s2} và ảnh radar khẩu độ tổng hợp từ vệ tinh Sentinel-1 \citeen{esa2024s1, torres2012}. Đối với Sentinel-2, nghiên cứu lựa chọn sản phẩm cấp độ L2A đã được hiệu chỉnh khí quyển để loại bỏ ảnh hưởng của sương mù và khí dung, đảm bảo giá trị phản xạ bề mặt chính xác. Các ảnh được thu thập trong giai đoạn mùa khô, cụ thể là tháng 01/2024 (kỳ trước) và tháng 02/2025 (kỳ sau), nhằm giảm thiểu tối đa sự che phủ của mây và đảm bảo tính tương đồng về điều kiện chiếu sáng giữa hai thời điểm. Toàn bộ quy trình truy xuất được thực hiện trên nền tảng điện toán đám mây Google Earth Engine, cho phép lọc và xử lý khối lượng lớn dữ liệu một cách hiệu quả. Các ảnh Sentinel-2 sau khi lọc mây với ngưỡng xác suất 50\% đã được dùng để tính toán các chỉ số thực vật quan trọng.

Song song đó, dữ liệu Sentinel-1 được thu thập ở chế độ IW với hai kênh phân cực VV và VH. Dữ liệu này được tiền xử lý các bước tiêu chuẩn gồm hiệu chỉnh quỹ đạo, khử nhiễu nhiệt, hiệu chỉnh bức xạ và hiệu chỉnh địa hình để tạo ra ảnh tán xạ ngược có đơn vị dB. Ưu điểm vượt trội của dữ liệu radar là khả năng xuyên thấu qua mây, cung cấp thông tin liên tục về cấu trúc bề mặt ngay cả khi dữ liệu quang học bị gián đoạn.

Thay vì đưa trực tiếp ảnh thô vào mô hình, em đã đã tiến hành trích xuất một tập hợp các đặc trưng giàu thông tin nhằm nâng cao khả năng phân loại của mô hình. Tổng cộng 27 đặc trưng đã được xây dựng cho mỗi điểm ảnh. Từ dữ liệu Sentinel-2, bốn kênh phổ cơ bản gồm B4, B8, B11 và B12 được sử dụng. Bên cạnh đó, ba chỉ số thực vật chuyên biệt được tính toán gồm NDVI (chỉ số thực vật chuẩn hóa) phản ánh mật độ xanh, NBR (tỷ số cháy chuẩn hóa) nhạy cảm với sự mất mát sinh khối, và NDMI (chỉ số độ ẩm) phản ánh hàm lượng nước trong lá. Từ dữ liệu Sentinel-1, hai kênh phân cực VV và VH cung cấp thông tin về độ nhám và cấu trúc thể tích của tán rừng. Để nắm bắt thông tin về sự thay đổi, em đã tính toán giá trị chênh lệch (delta) của tất cả các đặc trưng này giữa hai thời điểm nghiên cứu. Như vậy, vector đặc trưng đầu vào cho mô hình bao gồm giá trị tại thời điểm trước, thời điểm sau và giá trị thay đổi của cả dữ liệu quang học và radar.
...
Kết quả phân tích phân bố cho thấy các chỉ số thực vật delta ($\Delta$NDVI, $\Delta$NBR, $\Delta$NDMI) thể hiện khả năng phân biệt vượt trội giữa các lớp biến động, trong khi các kênh ra-đa ($\Delta$VV, $\Delta$VH) có sự chồng chéo đáng kể nhưng vẫn mang lại thông tin bổ sung giá trị trong điều kiện mây che phủ.

\subsection{Phương pháp nghiên cứu}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{img/flowchart.png}
    \caption{Sơ đồ quy trình tổng quát của nghiên cứu}
    \label{fig:flowchart}
\end{figure}

Quy trình nghiên cứu tổng thể được thiết kế gồm năm giai đoạn chính: (1) Thu thập và tiền xử lý dữ liệu vệ tinh đa nguồn, (2) Trích xuất hệ đặc trưng tối ưu, (3) Xây dựng bộ dữ liệu mẫu huấn luyện, (4) Huấn luyện và tinh chỉnh mô hình học sâu, và (5) Áp dụng mô hình để thành lập bản đồ biến động rừng.
...
Bộ dữ liệu thực địa bao gồm 2.630 điểm mẫu được biên tập kỹ lưỡng, với sự phân bố tương đối cân bằng giữa bốn lớp đối tượng nghiên cứu. Việc đảm bảo cân bằng dữ liệu đóng vai trò quan trọng trong việc hạn chế hiện tượng thiên lệch (bias) của mô hình về phía các lớp chiếm đa số.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{img/chapter3/samples.png}
\caption{Bản đồ phân bố các điểm dữ liệu thực địa trên khu vực nghiên cứu tỉnh Cà Mau}
\label{fig:ground_truth_map}
\end{figure}

Mạng lưới điểm mẫu được thiết lập phân tán rộng khắp trên toàn bộ phạm vi địa lý của khu vực nghiên cứu, bao gồm các vùng sinh thái đặc trưng khác nhau. Chiến lược lấy mẫu này đảm bảo tính đại diện về mặt không gian cho các loại hình biến động rừng, tăng cường khả năng tổng quát hóa của mô hình.

Để khai thác thông tin ngữ cảnh không gian, thay vì phân loại từng điểm ảnh độc lập, nghiên cứu sử dụng phương pháp trích xuất các ô kích thước $3 \times 3$ điểm ảnh xung quanh mỗi điểm mẫu. Như vậy, mỗi mẫu dữ liệu đầu vào là một khối tensor có kích thước $(27, 3, 3)$. Trước khi đưa vào mô hình, toàn bộ dữ liệu được chuẩn hóa theo phương pháp Z-score để đưa các đặc trưng về cùng một thang đo, giúp quá trình tối ưu hóa hội tụ nhanh hơn.

Dữ liệu được chia thành hai tập độc lập: tập huấn luyện/kiểm định (80\%) và tập kiểm tra (20\%). Việc phân chia được thực hiện theo phương pháp phân tầng để đảm bảo tỷ lệ các lớp trong mỗi tập con tương đương với tập dữ liệu gốc. Tập kiểm tra được giữ hoàn toàn riêng biệt và chỉ được sử dụng một lần duy nhất để đánh giá hiệu năng cuối cùng của mô hình.

Nghiên cứu đề xuất một kiến trúc mạng nơ-ron tích chập được thiết kế riêng cho bài toán này, tối ưu hóa sự cân bằng giữa độ chính xác và chi phí tính toán. Kiến trúc mạng bao gồm các thành phần chính sau:

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{img/cnn_architecture.png}
    \caption{Sơ đồ kiến trúc mạng CNN đề xuất}
    \label{fig:cnn_architecture}
\end{figure}

Kiến trúc mạng nơ-ron tích chập (CNN) đề xuất được thiết kế tối ưu cho dữ liệu viễn thám độ phân giải trung bình, kết hợp các kỹ thuật hiện đại như Global Average Pooling và Dropout để kiểm soát hiện tượng quá khớp (overfitting) trên tập dữ liệu mẫu hạn chế, đồng thời đảm bảo hiệu suất tính toán.
...
Các thực nghiệm đánh giá kích thước cửa sổ trượt (patch size) cho thấy kích thước 3x3 điểm ảnh đạt hiệu suất phân loại tối ưu (Accuracy 98,86\%). Kết quả này chỉ ra rằng ngữ cảnh không gian cục bộ trong phạm vi 30m x 30m là phù hợp nhất để đặc tả các đối tượng rừng ngập mặn vốn có tính phân mảnh cao tại khu vực nghiên cứu, trong khi các kích thước lớn hơn có xu hướng bị nhiễu từ các đối tượng lân cận.
...
Biểu đồ so sánh độ chính xác xác nhận xu hướng giảm dần của hiệu suất khi kích thước patch tăng lên hoặc giảm xuống so với kích thước tối ưu 3x3. Điều này củng cố lựa chọn kích thước đầu vào cho mô hình, cân bằng giữa việc nắm bắt thông tin ngữ cảnh và duy trì độ chi tiết của đối tượng.
...
Phân tích định lượng vai trò của từng nguồn dữ liệu khẳng định tính ưu việt của phương pháp đa nguồn. Sự kết hợp giữa thông tin quang phổ từ Sentinel-2 và thông tin cấu trúc từ Sentinel-1 giúp cải thiện đáng kể độ chính xác phân loại, đặc biệt trong việc giảm thiểu sự nhầm lẫn giữa các đối tượng có đặc điểm phổ tương tự.
...
Kết quả so sánh trực quan minh họa rõ lợi ích của việc tích hợp dữ liệu. Nhóm đặc trưng kết hợp (S1+S2) đạt được sự ổn định cao nhất và vượt trội hơn hẳn so với các trường hợp chỉ sử dụng một nguồn dữ liệu duy nhất, khẳng định giả thuyết về tính bổ sung giữa hai loại cảm biến.
...
Diễn biến của hàm mất mát (loss) và độ chính xác (accuracy) trong quá trình huấn luyện cho thấy mô hình hội tụ ổn định sau khoảng 70 epoch. Không quan sát thấy dấu hiệu của hiện tượng quá khớp nghiêm trọng, chứng tỏ các kỹ thuật điều chuẩn (regularization) đã được áp dụng hiệu quả.
...
Kết quả đánh giá trên tập kiểm tra độc lập cho thấy mô hình đạt độ tin cậy cao với tỷ lệ phân loại sai thấp (1,14\%). Các sai số chủ yếu tập trung vào việc phân biệt giữa lớp Rừng ổn định và Mất rừng, nguyên nhân có thể do sự tương đồng về đặc trưng phổ tại các khu vực rừng suy thoái nhẹ hoặc các điểm ảnh hỗn hợp tại vùng biên.
...
Bản đồ biến động rừng phản ánh cấu trúc không gian của lớp phủ, với diện tích rừng ổn định tập trung chủ yếu tại các khu vực Vườn Quốc gia và rừng phòng hộ. Ngược lại, các điểm nóng mất rừng phân bố phân tán, thường gắn liền với các khu vực tiếp giáp với hoạt động nuôi trồng thủy sản và ven biển.
...
Tại khu vực trọng điểm Vườn Quốc gia Mũi Cà Mau, kết quả phân loại thể hiện chi tiết các biến động quy mô nhỏ. Mô hình nhận diện chính xác các khu vực mất rừng dọc theo hệ thống kênh mương và bờ bao, cũng như ghi nhận sự phục hồi tự nhiên của thảm thực vật tại các bãi bồi ven biển.

Kết quả này cho thấy một xu hướng biến động phức tạp: mặc dù có sự mất mát rừng do các nguyên nhân tự nhiên và nhân sinh (chuyển đổi mục đích sử dụng), nhưng cũng có những tín hiệu tích cực từ việc phục hồi rừng.
...
Thống kê diện tích cho thấy xu hướng suy giảm ròng của tài nguyên rừng trong giai đoạn nghiên cứu, với tỷ lệ mất rừng (4,48\%) cao hơn so với tỷ lệ phục hồi (3,04\%). Sự chênh lệch này đặt ra yêu cầu cấp thiết về các biện pháp quản lý và bảo vệ rừng chặt chẽ hơn.
...
So sánh tương quan với các nghiên cứu trước đây cho thấy phương pháp đề xuất đạt độ chính xác vượt trội (98,86\%). Sự cải thiện này chủ yếu đến từ việc khai thác dữ liệu ảnh vệ tinh độ phân giải cao (10m) kết hợp với kiến trúc học sâu được tối ưu hóa cho điều kiện địa phương, khắc phục được các hạn chế của các sản phẩm toàn cầu có độ phân giải thấp.
