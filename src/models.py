...
class SpatialContextCNN(nn.Module):
    ...
    Example:
        >>> model = SpatialContextCNN(in_channels=14)
...
class MultiScaleCNN(nn.Module):
    ...
    Example:
        >>> model = MultiScaleCNN(in_channels=14)
        >>> x = torch.randn(1, 14, 128, 128)
        >>> y = model(x)
        >>> print(y.shape)  # torch.Size([1, 1, 128, 128])
    """
...
class ShallowUNet(nn.Module):
    ...
    Example:
        >>> model = ShallowUNet(in_channels=14)
        >>> x = torch.randn(1, 14, 128, 128)
        >>> y = model(x)
        >>> print(y.shape)  # torch.Size([1, 1, 128, 128])
    """
...
def get_model(model_name: str, in_channels: int = 14) -> nn.Module:
    """
    ...
    Example:
        >>> model = get_model('spatial_cnn', in_channels=14)
...
def print_model_summary(model: nn.Module, input_size: Tuple[int, int, int, int] = (1, 14, 128, 128)):
    """
    Print model summary
...
if __name__ == "__main__":
    ...
    for model_name in models:
        ...
        model = get_model(model_name, in_channels=14)
        print_model_summary(model)

        # Test forward pass
        x = torch.randn(2, 14, 128, 128)  # Batch of 2
        y = model(x)
