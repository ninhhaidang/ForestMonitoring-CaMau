...
class RasterPredictor:
    ...
    def predict_raster(self, feature_stack: np.ndarray,
        ...
        classification_flat = np.full(total_pixels, -1, dtype=np.int8)  # -1 for NoData
        ...
        for i in range(n_batches):
            ...
            # Predict
            batch_pred = self.model.predict(batch_features)
            batch_proba = self.model.predict_proba(batch_features)[:, 1]

            # Store predictions
            classification_flat[batch_indices] = batch_pred
            probability_flat[batch_indices] = batch_proba
        ...
        for cls, count in zip(unique_classes, class_counts):
            logger.info(f"    Class {cls}: {count:,} pixels ({count/valid_pixels*100:.2f}%)")
    ...
    def save_classification_raster(self, metadata: dict, output_path=None):
        """
        Save binary classification raster
        ...
        logger.info("\n" + "="*70)
        logger.info("SAVING CLASSIFICATION RASTER")
        logger.info("="*70)
    ...
    def get_prediction_summary(self) -> dict:
        ...
        valid_mask = self.classification_map != -1
