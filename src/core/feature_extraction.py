...
class FeatureExtraction:
    """Class to handle feature extraction from satellite imagery"""
    ...
    def create_valid_mask(s2_before: np.ndarray, s2_after: np.ndarray,
        ...
        """
        Create mask for valid pixels (allows S1-only pixels for cloud coverage)
        ...
        Returns:
            Tuple of (valid_mask, s2_mask)
            - valid_mask: Boolean mask (True = has S1 or S2 data)
            - s2_mask: Boolean mask (True = has S2 data, for imputation)
        """
        ...
        # S2 mask: has both S2 before and after
        s2_mask = valid_s2_before & valid_s2_after

        # S1 mask: has both S1 before and after
        s1_mask = valid_s1_before & valid_s1_after

        # Relaxed valid mask: has S1 OR S2 (allows S1-only pixels for cloud coverage)
        valid_mask = s1_mask | s2_mask

        return valid_mask, s2_mask

    def extract_features(self, s2_before: np.ndarray, s2_after: np.ndarray,
                         s1_before: np.ndarray, s1_after: np.ndarray,
                         nodata_value: float = 0) -> Tuple[np.ndarray, np.ndarray]:
        """
        Extract all 27 features from satellite data
        ...
        # Create valid mask (relaxed: allows S1-only pixels)
        logger.info("\nCreating valid pixel mask (relaxed for cloud coverage)...")
        self.valid_mask, s2_mask = self.create_valid_mask(
            s2_before, s2_after, s1_before, s1_after, nodata_value
        )
        valid_pixels = np.sum(self.valid_mask)
        s2_pixels = np.sum(s2_mask)
        s1_only_pixels = valid_pixels - s2_pixels
        total_pixels = self.valid_mask.size
        logger.info(f"  ✓ Total valid pixels: {valid_pixels:,} / {total_pixels:,} ({valid_pixels/total_pixels*100:.2f}%)")
        logger.info(f"  ✓ Pixels with S2 data: {s2_pixels:,} ({s2_pixels/total_pixels*100:.2f}%)")
        logger.info(f"  ✓ S1-only pixels (cloudy): {s1_only_pixels:,} ({s1_only_pixels/total_pixels*100:.2f}%)")
        ...
        # Calculate median values for S2 imputation (for cloudy pixels)
        s2_before_imputed = s2_before.copy()
        s2_after_imputed = s2_after.copy()

        if s1_only_pixels > 0:
            logger.info("  - Imputing S2 values for cloudy pixels using median...")
            for i in range(s2_before.shape[0]):
                # Calculate median from valid S2 pixels
                valid_values_before = s2_before[i][s2_mask]
                valid_values_after = s2_after[i][s2_mask]

                if len(valid_values_before) > 0:
                    median_before = np.median(valid_values_before)
                    median_after = np.median(valid_values_after)

                    # Fill missing values with median
                    s2_before_imputed[i][~s2_mask & self.valid_mask] = median_before
                    s2_after_imputed[i][~s2_mask & self.valid_mask] = median_after

        # S2 Before (7 features)
        logger.info("  - Adding S2 Before bands (7 features)")
        for i, band_name in enumerate(S2_BANDS['names']):
            feature_list.append(s2_before_imputed[i])

        # S2 After (7 features)
        logger.info("  - Adding S2 After bands (7 features)")
        for i, band_name in enumerate(S2_BANDS['names']):
            feature_list.append(s2_after_imputed[i])

        # S2 Delta (7 features)
        logger.info("  - Calculating S2 Delta (7 features)")
        s2_delta = self.calculate_delta(s2_before_imputed, s2_after_imputed)
        for i, band_name in enumerate(S2_BANDS['names']):
    ...
    def get_feature_summary(self) -> Dict:
        ...
        if self.feature_stack is None:
            logger.warning("Features not extracted yet")
            return {}
