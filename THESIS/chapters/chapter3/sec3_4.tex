...
Với bộ dữ liệu có quy mô nhỏ, nghiên cứu kết hợp ba kỹ thuật điều chuẩn: (1) \textbf{Chuẩn hóa theo lô} \cite{ioffe2015} sau mỗi lớp tích chập và fully-connected; (2) \textbf{Dropout 70\%} \cite{srivastava2014} — tỷ lệ cao do tỷ lệ mẫu/tham số thấp (~72:1); (3) \textbf{Phân rã trọng số} $\lambda = 10^{-3}$ thông qua optimizer AdamW.
...
Nghiên cứu sử dụng AdamW \cite{loshchilov2019} — biến thể cải tiến của Adam với phân rã trọng số tách rời. ReduceLROnPlateau scheduler tự động giảm learning rate (factor=0.5, patience=10) khi validation loss không cải thiện.
