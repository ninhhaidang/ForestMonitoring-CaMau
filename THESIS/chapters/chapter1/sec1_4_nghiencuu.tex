...
\textbf{Phương pháp viễn thám thế hệ đầu:}
...
\textbf{Phương pháp học máy truyền thống:}
...
\textbf{Nghiên cứu sử dụng học máy truyền thống:}
...
\textbf{Nghiên cứu sử dụng học sâu:}

Kussul và cộng sự \citeen{kussul2017} áp dụng CNN cho phân loại cây trồng từ Sentinel-2 và đạt accuracy 94.5\%, cao hơn đáng kể so với Random Forest. Zhang và cộng sự \citeen{zhang2016} giới thiệu các kiến trúc CNN phổ biến (AlexNet, VGGNet, ResNet) và ứng dụng của chúng trong viễn thám.
...
\begin{enumerate}
    ...
    \item \textbf{Tích hợp SAR và Optical trong Deep Learning:} Việc tích hợp SAR và Optical trong bối cảnh Deep Learning vẫn còn nhiều thách thức và còn thiếu các khảo sát tối ưu hóa fusion trong kiến trúc CNN.
\end{enumerate}

\textbf{Điểm mới của đồ án:}

Xuất phát từ những khoảng trống nghiên cứu đã nêu, đồ án này hướng tới bốn mục tiêu chính:

\begin{enumerate}
    \item \textbf{Phát triển kiến trúc CNN phù hợp cho bộ dữ liệu nhỏ:} Thiết kế mô hình lightweight (xấp xỉ 36K tham số), áp dụng các kỹ thuật điều chuẩn mạnh như chuẩn hóa theo lô, Dropout và phân rã trọng số.

    \item \textbf{Triển khai quy trình đánh giá khoa học chặt chẽ:} Bao gồm việc chia mẫu ngẫu nhiên phân tầng, thực hiện 5-Fold Stratified Cross Validation và giữ lại một fixed test set (20\%).

    \item \textbf{Tối ưu hóa phương pháp fusion:} Tích hợp Sentinel-1 và Sentinel-2 ở cấp độ feature, trích xuất 27 features tổng cộng (21 features từ S2 và 6 features từ S1).
