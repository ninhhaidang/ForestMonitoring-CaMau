...
\hline
\textbf{Nguồn dữ liệu} & \textbf{Độ phân giải} & \textbf{Kỳ ảnh} &  \textbf{Ghi chú} \\
\hline
Sentinel-2 trước & 10m & 30/01/2024 & 7 băng \\
\hline
Sentinel-2 sau & 10m & 28/02/2025 & 7 băng \\
\hline
Sentinel-1 trước & 10m & 04/02/2024 & 2 băng \\
\hline
Sentinel-1 sau & 10m & 22/02/2025 & 2 băng \\
\hline
...
Dữ liệu Sentinel-2 được truy xuất từ bộ dữ liệu \texttt{COPERNICUS/S2\_SR\_HARMONIZED} — sản phẩm đã được hiệu chỉnh khí quyển và đồng nhất hóa giữa các cảm biến Sentinel-2A và 2B. Quy trình xử lý bắt đầu bằng việc lọc theo không gian và thời gian để chọn các cảnh phủ khu vực nghiên cứu trong ngày chỉ định, sau đó loại bỏ mây sử dụng bộ sưu tập \texttt{S2\_CLOUD\_PROBABILITY} với ngưỡng xác suất 50\%. Tiếp theo, 4 băng cần thiết (B4-Red, B8-NIR, B11-SWIR1, B12-SWIR2) được trích xuất, chuyển đổi sang giá trị phản xạ và tính toán 3 chỉ số thực vật NDVI, NBR, NDMI. Cuối cùng, các tiles được mosaic để tạo ảnh liền mạch phủ toàn bộ khu vực nghiên cứu.
...
\begin{subfigure}[b]{0.32\textwidth}
    ...
    \caption{VH (SAR)}
\end{subfigure}
\caption{Các băng quang học, chỉ số thực vật và dữ liệu ra-đa kỳ trước (T1 - 01/2024)}
...
\begin{subfigure}[b]{0.32\textwidth}
    ...
    \caption{VH (SAR)}
\end{subfigure}
\caption{Các băng quang học, chỉ số thực vật và dữ liệu ra-đa kỳ sau (T2 - 02/2025)}
